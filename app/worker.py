...
async def process_update(ctx, update_dict: dict):
    ...
    async with async_session_maker() as session:
        ...
        await redis_client.set(f"user_role:{telegram_id}", profile["role"], ex=86400)
    ...
    if not has_consented:
        ...
        keyboard = {
            "inline_keyboard": [
                [
                    {
                        "text": "📄 Privacy Policy",
                        "url": f"https://{settings.DOMAIN_NAME}/legal/Privacy_Policy.pdf",
                    },
                    {
                        "text": "📄 Terms of Use",
                        "url": f"https://{settings.DOMAIN_NAME}/legal/Terms_of_Use.pdf",
                    },
                ],
                [
        ...
        await telegram_service.send_message(chat_id, contract_text, keyboard)
